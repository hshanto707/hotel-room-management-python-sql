...
from app.config import PRIMARY_COLOR, SECONDARY_COLOR, APP_NAME  # Import colors and project name from config
...
class DashboardView:
    ...
    def create_header(self, parent):
        ...
        profile_button = tk.Menubutton(header_frame, text="Profile", bg=PRIMARY_COLOR, fg=SECONDARY_COLOR, 
                                       font=("Helvetica", 12), relief="flat", cursor="hand2")
        profile_button.pack(side="right", padx=20)

        profile_menu = tk.Menu(profile_button, tearoff=0, bg="white", fg=PRIMARY_COLOR)
        profile_menu.add_command(label="Profile", command=self.switch_to_profile)
